  ...
  methods: {
    ...
    */
    async fetchPermissionList() {
      const { data: result } = await this.$http.menu.getPermissionList()
    ...
    */
    removePermission(permission) {
      this.$messageBox('此操作将永久删除该记录, 是否继续?', '提示', {
        type: 'warning'
      })
        .then(async () => {
          const { data: result } = await this.$http.menu.removePermission(
            permission.id
          )
    ...
    addOrUpdatePermission() {
      this.$refs.permission.validate(async (valid) => {
        if (valid) {
          ...
          const { data: result } = await this.$http.menu[
